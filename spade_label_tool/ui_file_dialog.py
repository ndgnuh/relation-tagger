...
class UIFileDialog(UIWindow):
    ...
    def __init__(self,
                 ...
                 object_id: Union[ObjectID, str] = ObjectID(
                     '#file_dialog', None),
        ...
        # type: Union[UIConfirmationDialog, None]
        ...
        text_line_rect = pygame.Rect(
            10, 40, self.get_container().get_size()[0] - 20, -1)
    ...
    def _process_file_path_entry_events(self, event):
        ...
        if self._validate_file_path(entered_file_path):
            ...
            elif not self.allow_existing_files_only and entered_file_path.parent.exists() and (
                    not entered_file_path.is_dir() or self.allow_picking_directories):
                self.ok_button.enable()
            else:
    ...
    def _process_ok_cancel_events(self, event):
        ...
        if (event.type == UI_BUTTON_PRESSED and event.ui_element == self.ok_button
            ...
            pygame.event.post(pygame.event.Event(
                UI_FILE_DIALOG_PATH_PICKED, event_data))
            self.kill()
